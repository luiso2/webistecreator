...
- La forja permanente de Railway sondea `/api/public/queue` cada 10 s y cada replica reclama un solo item con `POST /api/public/queue/claim {id}`. Solo procesa si recibe `200`; un `409` indica que otra replica ya lo tomó. El Durable Object serializa reclamos, reintentos, finalizaciones y upserts del registro. Cada item tiene un presupuesto duro de **9 min**; si una réplica muere, vuelve a estar disponible tras **10 min** (auto-rescate), y el Cron del panel revisa ese estado cada 5 min.
