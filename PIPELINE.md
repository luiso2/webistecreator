...
- **Búsqueda filtrada del panel**: un item con `request.type: "discovery"`, `niche`, `location`, `count` (1-5) y `require_no_website: true`. Busca SOLO dentro de la ubicación solicitada, por ejemplo `electricistas` en `España`; no cae a `config.json`, a otro país ni a negocios con website propio. Los candidatos de una misma búsqueda se construyen en paralelo con un límite de tres workers.
...
- API con key (header `x-sf-key`): GET `/api/state` (registry + queue), POST `/api/queue` con `{input}` para un negocio puntual o `{request:{type:"discovery", niche, location, count}}` para encontrar 1-5 negocios sin website, POST `/api/queue/done` {id}, POST `/api/registry` (array completo).
- API publica (autorizada por el usuario 2026-07-16, para que la rutina cloud trabaje sin credenciales): GET `/api/public/queue` (items pending: id/input/request/created), POST `/api/public/queue/claim` `{id}` (devuelve un `claim_token` de ese intento), POST `/api/public/queue/progress` `{id, token, stage, note?}` y POST `/api/public/queue/done` `{id, token, slug?, name?, url_demo?, sites?}`. Antes de research se usa POST `/api/public/queue/candidates/reserve`; al publicar se llama `candidates/complete` y un fallo libera con `candidates/release`. Las reservas comparan place id de Maps, telefono, nombre+ciudad y slug dentro del Durable Object: dos replicas no pueden construir la misma identidad y un negocio ya registrado se reutiliza. `sites` admite los 1-5 resultados `{slug,name,url_demo,dm?}` de una búsqueda filtrada; url_demo se valida server-side contra *.odd-forest-9504.workers.dev. El token evita que un worker antiguo pueda cerrar o avanzar un reintento nuevo.

## Forja de la cola (rutinas siteforge-queue)
- La forja permanente de Railway sondea `/api/public/queue` cada 10 s y cada replica reclama un solo item con `POST /api/public/queue/claim {id}`. Solo procesa si recibe `200`; un `409` indica que otra replica ya lo tomó. El Durable Object serializa reclamos, reintentos, finalizaciones, reservas de identidad y upserts del registro. Cada item tiene un presupuesto duro de **510 s (8.5 min)**; si una réplica muere, vuelve a estar disponible tras **9 min** (auto-rescate), y el Cron del panel revisa ese estado cada minuto.
...
- **Item `request.type: "discovery"`**: leer el filtro estructurado, buscar candidatos del `niche` exclusivamente en `location`, y verificar website propio antes de gastar el cupo. `require_no_website` es siempre true: un dominio propio descarta al candidato, mientras que Booksy, GlossGenius, Google Business, Facebook, Instagram o WhatsApp no cuentan como website propio. Wix, Squarespace, Weebly, WordPress, Webflow, GoDaddy Sites y Canva **sí** cuentan como website aunque usen un subdominio hospedado. Reservar identidades antes de construir, reutilizar la URL exacta de Maps durante research, construir hasta `count` candidatos válidos en paralelo (maximo 3), usar reemplazos solo cuando una ficha falle y nunca publicar más de `count` demos. Las búsquedas genéricas consultan dos nichos a la vez y como máximo cuatro, para no consumir el SLA completo solo en discovery. Hacer el upsert del registro por CADA uno y cerrar el id padre UNA vez con `sites: [{slug,name,url_demo,dm}, ...]`. Si no se encuentra ninguno dentro del filtro, cerrar como failed con un motivo; nunca ampliar el país, cambiar el nicho ni llenar con negocios que ya tienen web.
...
- El repo esta conectado a Cloudflare Workers Builds: cada push a main redeploya `siteforge-demos` automaticamente (deploy command: `npx wrangler deploy -c demos/wrangler.jsonc`). La forja toca `demos/.forge-deploy` en cada publicación para activar el trigger incluso si la conexión usa filtros por ruta. Asi la rutina cloud publica demos sin credenciales.
