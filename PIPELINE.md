...
- API publica (autorizada por el usuario 2026-07-16, para que la rutina cloud trabaje sin credenciales): GET `/api/public/queue` (items pending: id/input/request/created), POST `/api/public/queue/claim` `{id}` (devuelve un `claim_token` de ese intento), POST `/api/public/queue/progress` `{id, token, stage, note?}` y POST `/api/public/queue/done` `{id, token, slug?, name?, url_demo?, sites?}`. `sites` admite los 1-3 resultados `{slug,name,url_demo,dm?}` de una búsqueda filtrada; url_demo se valida server-side contra *.odd-forest-9504.workers.dev. El token evita que un worker antiguo pueda cerrar o avanzar un reintento nuevo.
