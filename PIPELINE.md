...
- API publica (autorizada por el usuario 2026-07-16, para que la rutina cloud trabaje sin credenciales): GET `/api/public/queue` (items pending: id/input/request/created), POST `/api/public/queue/claim` `{id}` (devuelve un `claim_token` de ese intento), POST `/api/public/queue/progress` `{id, token, stage, note?}` y POST `/api/public/queue/done` `{id, token, slug?, name?, url_demo?, sites?}`. `sites` admite los 1-5 resultados `{slug,name,url_demo,dm?}` de una búsqueda filtrada; url_demo se valida server-side contra *.odd-forest-9504.workers.dev. El token evita que un worker antiguo pueda cerrar o avanzar un reintento nuevo.

## Forja de la cola (rutinas siteforge-queue)
- La forja permanente de Railway sondea `/api/public/queue` cada 10 s y cada replica reclama un solo item con `POST /api/public/queue/claim {id}`. Solo procesa si recibe `200`; un `409` indica que otra replica ya lo tomó. El Durable Object serializa reclamos, reintentos, finalizaciones y upserts del registro. Cada item tiene un presupuesto duro de **9 min**; si una réplica muere, vuelve a estar disponible tras **12 min** (auto-rescate), y el Cron del panel revisa ese estado cada 5 min.
